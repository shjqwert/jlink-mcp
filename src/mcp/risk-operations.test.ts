...
test("R4 executor audits, atomically consumes, synchronizes event, and rejects replay", async () => {
  ...
  const endpoint = approvalBrokerEndpoint(`${root}-${challenge.challengeId}`, temp.stateRoot);
  const broker = await startApprovalBrokerIpc(endpoint, root, temp.stateRoot, () => true);
  try {
    await approve(root, temp.stateRoot, challenge.challengeId);
    let executions = 0;
    let events = 0;
    let revalidations = 0;
    const hooks = {
      revalidate: () => { revalidations += 1; return input; },
    ...
    const first = await executeR4Operation({ challengeId: challenge.challengeId, cwd: root }, hooks);
    assert.equal(first.ok, true, first.ok ? undefined : `${first.error.code}: ${first.error.message}`);
    assert.equal(executions, 1);
    assert.equal(events, 1);
    assert.equal(revalidations, 1);
    const replay = await executeR4Operation({ challengeId: challenge.challengeId, cwd: root }, hooks);
...
test("missing, unapproved, mismatched, expired, and execute-time R5 approvals make zero executor calls", async () => {
  const temp = await tempProject();
  const root = temp.root;
  const endpoint = approvalBrokerEndpoint(`${root}-reject`, temp.stateRoot);
  const broker = await startApprovalBrokerIpc(endpoint, root, temp.stateRoot, () => true);
  let executions = 0;
  ...
  try {
    ...
    const unapprovedInput = planInput("erase", {});
    const unapproved = planR4Operation(unapprovedInput);
    const unapprovedResult = await executeR4Operation({ challengeId: unapproved.challengeId, cwd: root }, { revalidate: () => unapprovedInput, execute });
    assert.equal(unapprovedResult.ok ? null : unapprovedResult.error.code, "approval_required");

    const changedInput = planInput();
    const changed = planR4Operation(changedInput);
    await approve(root, temp.stateRoot, changed.challengeId);
    const changedResult = await executeR4Operation({ challengeId: changed.challengeId, cwd: root }, {
    ...
    await approve(root, temp.stateRoot, r5.challengeId);
    const r5Result = await executeR4Operation({ challengeId: r5.challengeId, cwd: root }, {
    ...
    await approve(root, temp.stateRoot, expired.challengeId);
    await delay(5100);
    const expiredResult = await executeR4Operation({ challengeId: expired.challengeId, cwd: root }, { revalidate: () => expiredInput, execute });
...
async function approve(cwd: string, stateRoot: string, challengeId: string): Promise<void> {
  assert.equal(await runApprovalBrokerCli([challengeId], cwd, stateRoot, interactiveIo(challengeId)), 0);
}

function interactiveIo(challengeId: string) {
  return {
...
async function tempProject(): Promise<{ root: string; stateRoot: string; remove(): Promise<void> }> {
  ...
  return { root, stateRoot: join(base, "approval-broker"), remove: () => rm(base, { recursive: true, force: true }) };
}
