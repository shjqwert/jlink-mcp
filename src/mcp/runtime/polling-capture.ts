...
import type { MemorySessionManager } from "./memory-session";
...
export class PollingCaptureRunner {
  ...
  private async run(job: PollingJob): Promise<void> {
    ...
    try {
      await this.memorySessions.closeForTarget(job.input.target);
    } catch (error) {
      terminalError = normalizePollingError(error, "POLLING_SESSION_CLOSE_FAILED", true);
    }
