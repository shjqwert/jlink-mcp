...
export const TOOL_DESCRIPTIONS: Record<AgentToolName, string> = {
  ...
  hss_start: configuredTarget("Validate or start an automatic J-Link trace capture. The retained hss_start name selects HSS first, then background polling, then stop polling without a device/core allowlist. For new capture parameters, first call dryRun=true with the real variable set, rate, and duration; never send an empty variables array."),
  hss_status: configuredTarget("Report an automatic trace capture lifecycle, selected backend, and quality counters."),
  hss_stop: configuredTarget("Stop an active trace capture and finalize its trustworthy data prefix."),
  hss_recover: configuredTarget("Recover and index the trustworthy prefix of an interrupted trace capture."),
  debug_sequence_execute: configuredTarget("Synchronously execute multiple trace/read/write operations on fixed intervals over at least one second and wait until completion. Do not use for a single variable read or write."),
