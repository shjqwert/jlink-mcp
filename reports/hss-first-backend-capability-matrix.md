...
| --- | ---: | --- | --- | --- | --- |
| `jlink-hss` | 1 | preflight available; benchmark blocked | JScope project opens in asynchronous/HSS mode and `JLink_x64.dll` contains HSS symbols; no typed prototype or verified headless export/benchmark path yet | no | highest-rate host-side streaming |
...
- Default local probe keeps `jlink-hss` first in priority but reports it as `available-if-configured`, then falls back with explicit reason: `reports/hss-default-backend-probe.json`.
...
JScope/HSS exists and can be opened against the HM_C095 project, but automated benchmark/export is blocked because no supported start/stop/export CLI and no local typed HSS SDK header were found. Fallback success is not HSS success.
