...
- 身份门禁区分 production mode 与用户显式启动、仅当前进程有效的 acceptance mode。验收模式只允许精确候选 DLL/helper/adapter/ScriptFile、指定 target 和 `GetCaps → R3 reset → HSS` 验收；成功只生成待用户提升的 trust-manifest 候选，不自动开放生产 allowlist。
...
- `.jcap` 最终合同为 `capture.db + raw/*.bin + optional export`。采样原始段和版本化事件日志是权威数据；终态事件持久化并关闭 raw journal 后才构建和发布 `capture.db`，以保证首次索引与 rebuild 等价；不默认生成 JSON/JSONL/CSV。
